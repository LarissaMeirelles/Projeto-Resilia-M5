...
const userControl = {
    ...
    Login: async (req, res) => {
        ...
        try {
            ...
            // Query
            const sql = `SELECT ${conf.EM}, ${conf.PA} FROM ${conf.U} WHERE ${conf.EM} = ${email} AND ${conf.PA} = ${bcrypt.hash(senha, 8)};`;
            ...
            const token = jwt.sign({ id: atributos[0].u_id }, conf.JWT_PASSWORD, {
                expiresIn: 3600 // 1h
                // expiresIn: 600 // 10min
                // expiresIn: 60 // 1min
                // expiresIn: '7d' // 7 dias
